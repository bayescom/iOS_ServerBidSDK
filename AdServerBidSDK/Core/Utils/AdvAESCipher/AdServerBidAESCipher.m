...
NSData * adServerBidCipherOperation(NSData *contentData, NSData *keyData, CCOperation operation) {
...
NSString * adServerBidAesEncryptString(NSString *content, NSString *key) {
    ...
    NSData *encrptedData = adServerBidAesEncryptData(contentData, keyData);
...
NSString * adServerBidAesDecryptString(NSString *content, NSString *key) {
    ...
    NSData *keyData = [key dataUsingEncoding:NSUTF8StringEncoding];
    NSData *decryptedData = adServerBidAesDecryptData(contentData, keyData);
    return [[NSString alloc] initWithData:decryptedData encoding:NSUTF8StringEncoding];
}

NSData * adServerBidAesEncryptData(NSData *contentData, NSData *keyData) {
    ...
    return adServerBidCipherOperation(contentData, keyData, kCCEncrypt);
}

NSData * adServerBidAesDecryptData(NSData *contentData, NSData *keyData) {
    NSCParameterAssert(contentData);
    NSCParameterAssert(keyData);
    
    NSString *hint = [NSString stringWithFormat:@"The key size of AES-%lu should be %lu bytes!", kKeySize * 8, kKeySize];
    NSCAssert(keyData.length == kKeySize, hint);
    return adServerBidCipherOperation(contentData, keyData, kCCDecrypt);
}
