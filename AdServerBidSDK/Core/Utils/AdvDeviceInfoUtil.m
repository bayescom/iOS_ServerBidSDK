...
- (NSMutableDictionary *)getDeviceInfoWithMediaId:(NSString *)mediaId adspotId:(NSString *)adspotId {
    ...
    @try {
        if (self.cacheInfo) {
            ...
            // 冷启动获取一次
            [deviceInfo setValue:AdServerBidSdkVersion forKey:@"sdk_version"];
            [deviceInfo setValue:AdServerBidSdkAPIVersion forKey:@"version"];
...
- (NSMutableDictionary *)createDeviceEncinfoWithDic:(NSMutableDictionary *)deviceInfo {
    ...
    NSString *jsonEncrypt = adServerBidAesEncryptString(json, AdServerBidSDKSecretKey);
...
- (NSMutableDictionary *)saveLocalForOneMonth {
    ...
    NSString *idfa = [[NSUserDefaults standardUserDefaults] objectForKey:AdServerBidSDKIdfaKey];
    NSString *idfv = [[NSUserDefaults standardUserDefaults] objectForKey:AdServerBidSDKIdfvKey];
    
    NSInteger _timeout_stamp = [[[NSUserDefaults standardUserDefaults] objectForKey:AdServerBidSDKOneMonthKey] integerValue];
    ...
    if (![self isEmptyString:idfa] &&
        ...
        NSString *decryptIdfa = adServerBidAesDecryptString(idfa, AdServerBidSDKSecretKey);
        NSString *decryptIdfv = adServerBidAesDecryptString(idfv, AdServerBidSDKSecretKey);
        ...
        [self saveInfo:tempIdfa key:AdServerBidSDKIdfaKey];
        [self saveInfo:tempIdfv key:AdServerBidSDKIdfvKey];
        [self saveNumber:__number key:AdServerBidSDKOneMonthKey];
    }
...
//    [deviceInfo setValue:[AdvDeviceInfoUtil getNetwork] forKey:@"network"];
    ...
    NSString *carrier = [[NSUserDefaults standardUserDefaults] objectForKey:AdServerBidSDKCarrierKey];
    NSNumber *network = [[NSUserDefaults standardUserDefaults] objectForKey:AdServerBidSDKNetworkKey];
    
    NSInteger _timeout_stamp = [[[NSUserDefaults standardUserDefaults] objectForKey:AdServerBidSDKHourKey] integerValue];
    ...
    if (![self isEmptyString:carrier] &&
        ...
        NSString *decryptText = adServerBidAesDecryptString(carrier, AdServerBidSDKSecretKey);
        ...
        [self saveInfo:tempCarrier key:AdServerBidSDKCarrierKey];
        [self saveNumber:tempNetwork key:AdServerBidSDKNetworkKey];
        [self saveNumber:__number key:AdServerBidSDKHourKey];
    }
...
- (NSString *)getModel {
    ...
    NSString *model = [[NSUserDefaults standardUserDefaults] objectForKey:AdServerBidSDKModelKey];

    if ([self isEmptyString:model]) {
        model = @"";
    } else {
        NSString *decryptText = adServerBidAesDecryptString(model, AdServerBidSDKSecretKey);
        return decryptText;
    }
    ...
    @try {
        ...
        [self saveInfo:model key:AdServerBidSDKModelKey];
    } @catch (NSException *exception) {
...
- (void)saveInfo:(NSString *)info key:(NSString *)key {
    ...
    NSString *encryptText = adServerBidAesEncryptString(info, AdServerBidSDKSecretKey);
