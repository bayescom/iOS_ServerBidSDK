...
@interface AdServerBidRewardVideo ()
...
@implementation AdServerBidRewardVideo
...
- (void)adServerBidBaseAdapterBiddingEndWithWinSupplier:(AdvSupplier *_Nonnull)supplier {
    if (self.delegate && [self.delegate respondsToSelector:@selector(adServerBidBiddingEndWithPrice:)]) {
        [self.delegate adServerBidBiddingEndWithPrice:supplier.supplierPrice];
    }
...
//            ADV_LEVEL_INFO_LOG(@"并行: %@", adapter);
            ...
            ADV_LEVEL_INFO_LOG(@"串行 %@ %ld %ld", _adapter, (long)[_adapter tag], supplier.identifier.integerValue);
            // 设置代理
