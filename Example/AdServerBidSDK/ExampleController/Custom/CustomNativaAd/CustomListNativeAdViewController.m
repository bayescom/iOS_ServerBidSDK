...
@interface CustomListNativeAdViewController () <UITableViewDelegate, UITableViewDataSource, AdServerBidBaseAdspotDelegate>
...
- (void)loadBtnAction:(id)sender {
    _adspot = [[AdServerBidNativeExpress alloc] initWithAdspotId:@"10002698" viewController:self adSize:CGSizeMake(self.view.bounds.size.width, 300)];
...
- (void)adServerBidBaseAdspotFailedWithError:(NSError *)error {
    NSLog(@"%@", error);
}
